...
~60000 runs got it to R=7936 KB, B=3710 KB. 10000000 runs got it to R=7488 KB, B=1662 KB. Deminishing returns go brrr. A bunch are still the golden ratio which is cool. But actually just the top number got stuck so it was just ignoring the lower ones. Doing another 1M runs picking a random index to try each time got R=2960 KB, B=432 KB. Another 9M to R=2496KB, B=387KB. Pleasingly that's around 10% of the default std one where I was asking to aim for 90% empty space. So it's approching perfectly packed (other than the power of 2 thing)? 
